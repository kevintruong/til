...
The `PROT_READ | PROT_WRITE` options specify the virtual memory protection. The option PROC_EXEC (not used here) can be set to allow CPU execution of instructions in memory (e.g. this would be useful if you mapped an executable or library).
...
Easy -  Use `mmap` without a file - just specify the MAP_ANON and MAP_SHARED options!
